<template>
  <q-page class="q-pa-xl">
    <div class="row justify-center">
      ...
      <div v-for="juz in juzList" :key="juz.id" class="col-3 q-my-sm" clickable>
        ...
        <q-chip square class="q-ml-md">{{juz.juz_number}}</q-chip>     
          <div class="row">
            <q-item :to="{name:'surah', params:{id:surah.surahId, title:surah.name_simple}}" v-for="surah in juz.verse_mapping" :key="surah.surahId" class="col-3 q-my-sm" clickable v-ripple>
              ...
              <q-item-section>
                <q-item-label>{{ surah.verses}}</q-item-label>
                <q-item-label lines="1">{{ surah.verse }}</q-item-label>
                <q-item-label lines="2">{{ surah.name_simple }}</q-item-label>
                <q-item-label caption lines="3">{{ surah.translated_name }}</q-item-label>
              </q-item-section>          
            </q-item>
          </div>
        </q-list>
    ...
    const getSurahName = () => {
     juzList.value.forEach((juz)=>{   
        Object.entries(juz.verse_mapping).forEach(([surahId, verse])=>{
          ...
          juz.verse_mapping[surahId] = {surahId, verse, name_simple:surah.name_simple, translated_name:surah.translated_name.name }          
        })        
      })
    }
